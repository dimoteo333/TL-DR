...
def convert_to_unicode(text):
    if isinstance(text, str):
        ...
        raise ValueError("Unknown string type: {}".format(type(text)))
...
class Tokenizer(object):
    def __init__(self, vocab_file_path):
        ...
        with open(self.vocab_file_path, "r", encoding='utf-8') as file:
            while True:
                token = convert_to_unicode(file.readline())
                if not token:
                    break
...
class BertData():
    ...
    def preprocess(self, src, tgt, source_org, tgt_org, oracle_ids):
        ...
        #for test only
        # src_org_txt = source_org
        # tgt_org_txt = tgt_org
        return src_subtoken_idxs, labels, segments_ids, cls_ids, src_txt, tgt_txt #, src_org_txt, tgt_org_txt
...
def _format_to_bert(params):
    ...
    for d in jobs:
        ...
        # for test only
        # source_org, tgt_org = d['src_org'], d['tgt_org']

        if (args.oracle_mode == 'greedy'):
            oracle_ids = greedy_selection(source, tgt, 3)
        elif (args.oracle_mode == 'combination'):
            oracle_ids = combination_selection(source, tgt, 3)
        # for test only : source_org, tgt_org
        b_data = bert.preprocess(source, tgt, source_org, tgt_org, oracle_ids)
        if (b_data is None):
            continue
